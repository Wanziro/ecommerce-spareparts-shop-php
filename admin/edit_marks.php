...
{
  include "admin_protect.php";
  $sql = "SELECT * FROM vehicle_marks WHERE id='$id'";
...
{
  include "admin_protect.php";
  $sql = "SELECT * FROM vehicle_marks WHERE name='$name'";
  $statement = $conn->query($sql);
  $amount = 0;
  if ($statement->rowCount() == 1) {
    return true;
...
if (isset($_POST['edit'])) {
  ...
  if ($image_upload == '') {
    if (check_if_vehicle_exists($name)) {
      ...
      //update the data
      $up = "UPDATE vehicle_marks SET name='$name',vehicle_category='$category' WHERE id = '$id'";
      $statement = $conn->query($up);

      $up = "UPDATE vehicle_model SET vehicle_mark='$name' WHERE vehicle_mark = '$oldName'";
      $statement = $conn->query($up);

      $up = "UPDATE spare_part_categories SET vehicle_mark='$name' WHERE vehicle_mark = '$oldName'";
      $statement = $conn->query($up);

      $up = "UPDATE spare_parts SET vehicle_mark='$name' WHERE vehicle_mark = '$oldName'";
      $statement = $conn->query($up);

      $up = "UPDATE fuel SET vehicle_mark='$name' WHERE vehicle_mark = '$oldName'";
      $statement = $conn->query($up);

      $up = "UPDATE engine_type SET vehicle_mark='$name' WHERE vehicle_mark = '$oldName'";
      $statement = $conn->query($up);

      header("Location: v_marks.php");
    }
  } else {
    ...
    if ($name != '' && $image_upload != "" && $id != "") {
      ...
      if (!check_image_format($image_upload_type)) {
        ...
        if (move_uploaded_file($image_upload_loc, $folder . $random_image_upload)) {
          //update the data
          if ($name_exists) {
            $up = "UPDATE vehicle_marks SET logo='$random_image_upload',vehicle_category='$category' WHERE id = '$id'";
            $statement = $conn->query($up);
          } else {
            $up = "UPDATE vehicle_categories SET name='$name',image='$random_image_upload',vehicle_category='$category' WHERE id = '$id'";
            $statement = $conn->query($up);
          }
        ...
        <div class="container-fluid">
          ...
          <div class="row">
            ...
            <?php if (isset($_GET['edit'])) {
              $s = "SELECT * FROM vehicle_marks WHERE id=" . $_GET['edit'] . "";
              $stm = $conn->query($s);
              if ($stm->rowCount() > 0) {
            ...
            ?>
                  <div class="col-lg-12">
                    <div class="card shadow mb-4">
                      <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">Vehicle Marks / Edit / <?php echo $_GET['edit']; ?></h6>
                      </div>
                      <div class="card-body">
                        <?php echo $msg; ?>
                        <form method="post" action="#" enctype="multipart/form-data">
                          ...
                          <div class="form-group">
                            <label>Image <font color="red">(Leave this as empty if you dont want to change current mark logo)</font></label>
              ...
              <?php
                ...
                echo "Vehicle Mark with id of " . $_GET['edit'] . " does not exist.";
              }
              ?>
            <?php
            } ?>
