  ...
  if($name != '' && $category != "" && $mark != ""){
    if(check_if_vehicle_exists($name,$category,$mark,$model,$fuel,$engine,$s_category)){
      ...
      if(move_uploaded_file($image_upload_loc,$folder.$random_image_upload)){
        $sql = "INSERT INTO spare_parts(name,spare_part_category,vehicle_category,vehicle_mark,vehicle_model,fuel,engine,part_number,quantity,price,image) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
        $statement = $conn->prepare($sql);
        $statement->execute(array($name,$s_category,$category,$mark,$model,$fuel,$engine,$part_number,$quantity,$price,$random_image_upload));
        $msg = '';
      }else{
        ...
        <div class="container-fluid">       
          ...
          <div class="row">
            ...
            if($statement->rowCount() > 0){
                ...
                <?php
            }else{echo'<div class="col-xl-12 col-lg-12"><h2>No Spare parts Found</h2><br></div>';}
            ?>  
  ...
  <script>
    ...
    function getCats(v){
      $.ajax({
        url: "../ajax.php",
        method:"POST",
        data: {getVehicleSpCat:1,admin:1,engine:v.value,vehicleFuel:$("#allFuels").val(),vehicleModel:$("#allModels").val(),vehicleMark:$("#allMarks").val(),vehicle:$("#vCategory").val()},
        success: data => {
